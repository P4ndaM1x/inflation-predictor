...
indicators_labels = {
    ...
    2: 'Budżet państwa',  # dane od 2000 roku (od początku roku do końca okresu!)
...
for indicator_id, indicator_name in indicators_labels.items():
    ...
    # TODO szczególna implementacja ze względu na unikalną strukturę plików z surowymi danymi
    ...
    subindicators_labels[indicator_id] = {date: '; '.join([str(d[date]) for d in labels])
    ...
    data = {tuple(map(int, k)):
                {indicator_id: {key: None if value == '.' else float(str(value).replace(',', '.')) + indicator_offset
